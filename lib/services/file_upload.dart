...
class FileUpload {
  ...
  Future<ImagePath?> uploadImage(
      {required String selectedPath, String? deletePath}) async {
    try {
      ...
      var photo = await storage
          .child("products")
          .child("$dt.jpg")
          .putFile(File(selectedPath));
      String photoUrl = await photo.ref.getDownloadURL();
      return ImagePath(imageUrl: photoUrl, imagePath: photo.ref.fullPath);
    } catch (e) {
      return null;
    }
  }
