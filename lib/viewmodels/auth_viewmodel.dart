...
class AuthViewModel with ChangeNotifier {
  ...
  Future<void> getCartsUser() async {
    try {
      var response = await _cartRepository.getCartsUser(loggedInUser!.userId!);
      _carts = [];
      for (var element in response) {
        _carts.add(element.data());
      }
      _cartProduct = [];
      if (_carts.isNotEmpty) {
        var productResponse = await ProductRepository()
            .getProductFromList(_carts.map((e) => e.productId).toList());
        for (var element in productResponse) {
          _cartProduct!.add(element.data());
        }
      }
  ...
  Future<void> cartAction(CartModel? isCart, String productId) async {
    try {
      await _cartRepository.cart(isCart, productId, loggedInUser!.userId!);
      await getCartsUser();
      notifyListeners();
    } catch (e) {
      _carts = [];
